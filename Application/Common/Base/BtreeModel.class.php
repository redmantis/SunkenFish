 ...
 */
abstract class BtreeModel extends BModel {
    ...
    function __construct($data) {
        if ($data) {
            ...
            if (isset($data['checkRepepeat'])) {
                $this->checkRepepeat = $data['checkRepepeat'];
        }
        }
     ...
     public function addnew($data) {
        ...
        if ($this->checkRepepeat && $this->checkTagmark($map, $model[$this->viewpath])) {
            $rs = ['status' => 20000003, 'msg' => 'KeywordRepeat'];
        } else {
            $rs = parent::addnew($data);
        }
        return $rs;
    }
    ...
    public function updater($data) {
        ...
        $ck = $this->checkTagmark($map, $model[$this->viewpath]);
        if ($this->checkRepepeat && $ck) {
            return array('status' => 20000003, 'msg' => 'KeywordRepeat');
        }
        $idlist = $this->getsublistid($map, $id);
        if (in_array($model['parentid'], $idlist)) {
            return array('status' => 20000009, 'msg' => 'notMutualFatherElement');
        }
        $rs = parent::updater($data);
        return $rs;
    }
    ...
    public function getmodelbypath($map) {
        ...
        $id = $this->getidbytagmark($map);
        if ($id) {            
            return $this->getmodelbyid(['id' => $id,'selectlangue'=>$map['selectlangue']]);
        }
    }
     ...
     * @return int
     */
    public function getidbytagmark($data) {
        $map[$this->viewpath] = $data[$this->viewpath];
        $lang = getLangueInfo($data['selectlangue'], $data['sid_id']);
    ...
    /**
     * 获取完整分类路径 with viewpath
     * @param type $viewpath 栏目标签
     * @param type $byself 包含子栏目
     * @return type
     */
    public function getParentbyViewpath($viewpath, $byself = true) {
        $id= $this->getidbytagmark($viewpath);
        return $this->getParentPath($id,$byself);
    }
     ...
     * @param type $sid_id
     */
    public function getTree3($map) {
        $dl = $this->getfieldlist($map);
        $dtree = genTree5($dl);
        return $dtree;
    }
    ...
    public function del($map, $id) {
        ...
        if (count($idlst) == 0 || $this->deltree) {
            ...
            $rs = array('status' => 20000001, "msg" => 'DataDelFailedBySub');
        }
        return $rs;
    }
